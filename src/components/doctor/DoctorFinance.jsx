...
const DoctorFinance = () => {
    ...
    const getStatusBadge = (status) => {
        ...
        return <Badge className={`${styles[status] || 'bg-gray-50 text-gray-700 border-gray-100'} h-5 text-[10px] px-2 rounded-md font-medium border`} variant="outline">{status?.toUpperCase()}</Badge>;
    };
    ...
    const getWithdrawalStatusBadge = (status) => {
        ...
        return <Badge className={`${styles[status] || 'bg-gray-50 text-gray-700'} h-5 text-[10px] px-2 rounded-md font-medium border`} variant="outline">{status}</Badge>;
    }
        ...
        <div className="space-y-6 max-w-6xl mx-auto pb-10">
            <DoctorPageHeader icon={Wallet} title="Financeiro" subtitle="Gerencie seus recebimentos e fluxo de caixa.">
                <Popover>
                    ...
                    <PopoverContent align="end" className="w-[380px] p-0 rounded-lg max-h-[80vh] overflow-y-auto border-gray-200 shadow-lg">
                                    ...
                                    {paidTransactions.length > 0 ? (
                                        paidTransactions.map((t) => (
                                            <TableRow key={t.id} className="h-10 hover:bg-gray-50 border-b border-gray-100 last:border-0">
                                                ...
                                                <TableCell className="py-2 px-4">
                                                    <Badge variant="outline" className={`h-5 text-[10px] px-2 rounded-md font-medium border ${t.pagamento_status === 'pago' ? 'border-green-200 text-green-700 bg-green-50' : 'border-amber-200 text-amber-700 bg-amber-50'}`}>
                                                ...
                                                <TableCell className="text-right py-2 px-4">
                                                    <Button variant="outline" size="sm" onClick={() => setDetailTx(t)} className="h-7 gap-1 text-[11px] rounded-md border-gray-300 text-gray-700">
            ...
            <Dialog open={!!detailTx} onOpenChange={(o) => !o && setDetailTx(null)}>
                <DialogContent className="p-6 sm:max-w-[460px] rounded-lg border-gray-200">
                        ...
                        <div className="space-y-3 text-sm">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-0.5">
                                    ...
                                    <Badge variant="outline" className={`h-5 text-[10px] px-2 rounded-md font-medium border ${detailTx.pagamento_status === 'pago' ? 'border-green-200 text-green-700 bg-green-50' : 'border-amber-200 text-amber-700 bg-amber-50'}`}>
                    ...
                    <DialogFooter className="p-0 pt-4">
                        <Button variant="outline" onClick={() => setDetailTx(null)} className="h-9 text-xs rounded-md border-gray-300">Fechar</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>

            <Dialog open={isWithdrawOpen} onOpenChange={(o) => { setIsWithdrawOpen(o); if (!o) setSelectedGuideIds([]); }}>
                <DialogContent className="p-6 sm:max-w-[520px] rounded-lg border-gray-200">
                    ...
                    <div className="py-2 space-y-4">
                         <div className="text-xs bg-brand-50 text-brand-800 p-3 rounded-lg border border-brand-100">
                         ...
                         <div className="text-[11px] text-amber-700 bg-amber-50 border border-amber-100 rounded-lg p-2.5">
                         ...
                         <div className="text-[11px] text-brand-800 bg-brand-50 border border-brand-100 rounded-lg p-2.5">
                         ...
                         {paidTransactions.length === 0 ? (
                             ...
                             <div className="border border-gray-200 rounded-lg divide-y max-h-[280px] overflow-y-auto">
                    ...
                    <DialogFooter className="p-0 pt-4">
                        <Button variant="outline" onClick={() => { setIsWithdrawOpen(false); setSelectedGuideIds([]); }} className="h-9 text-xs rounded-md border-gray-300">Cancelar</Button>
                        <Button onClick={handleRequestWithdraw} disabled={requestingWithdraw || selectedGuideIds.length === 0 || !doctorData?.withdrawal_payment_method || !canRequestWithdraw} className="h-9 text-xs rounded-md bg-primary hover:bg-primary/90 text-primary-foreground shadow-sm">
            ...
            <Dialog open={!!detailSaque} onOpenChange={(o) => !o && setDetailSaque(null)}>
                <DialogContent className="p-6 sm:max-w-[480px] rounded-lg border-gray-200">
                    <DialogHeader className="p-0 pb-3">
                    ...
                    <DialogFooter className="p-0 pt-4">
                        <Button variant="outline" onClick={() => setDetailSaque(null)} className="h-9 text-xs rounded-md border-gray-300">Fechar</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
