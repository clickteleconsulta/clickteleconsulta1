...
const DoctorArea = () => {
    ...
    return (
        <div className="flex min-h-[100dvh] md:h-screen bg-gray-100 font-sans md:overflow-hidden">
            {/* ══════════════════════════════════════════════════════════════
                O TRILHO, NAS DUAS LARGURAS
                No celular ele é sempre só ícones, com 60px de largura, colado
                no alto enquanto o conteúdo rola. Não expande: o botão de
                expandir é escondido abaixo de md, porque 240px de menu numa
                tela de 375 deixariam 135 para o conteúdo.
                A lista é a MESMA de `menuSections` nas duas larguras — todos os
                destinos ficam alcançáveis no celular, sem nada atrás de um
                botão "Mais".
            ══════════════════════════════════════════════════════════════ */}
            <aside className={`w-[60px] ${expanded ? 'md:w-60' : 'md:w-[88px]'} bg-white border-r border-gray-200 flex flex-col py-4 md:py-8 gap-4 md:gap-6 z-20 flex-shrink-0 transition-all duration-300 ease-in-out sticky top-0 h-[100dvh] md:static md:h-auto`}>
                {/* Perfil + botão de expandir/recolher */}
                <div className={`flex items-center gap-3 px-2 md:px-4 ${expanded ? 'justify-between' : 'flex-col justify-center'}`}>
                    ...
                    <Tooltip delayDuration={0}>
                        <TooltipTrigger asChild>
                            <button
                                ...
                                className={`hidden md:flex items-center justify-center w-8 h-8 rounded-lg text-gray-400 hover:text-brand-600 hover:bg-brand-50 transition-all duration-200 flex-shrink-0 ${expanded ? '' : 'mt-1'}`}
                            >
                ...
                <nav className={`flex flex-col w-full flex-1 min-h-0 overflow-y-auto px-1.5 md:px-3 no-scrollbar ${expanded ? 'items-stretch' : 'items-center'}`}>
                            ...
                            {section.items.map((item) => {
                                ...
                                const link = (
                                    ...
                                    >
                                        ...
                                        {/* A lasquinha na borda esquerda só existe no desktop: num
                                            trilho de 60px ela cairia rente ao canto da tela, com meio
                                            pixel cortado pela rolagem do menu. No celular o item ativo
                                            já se distingue pelo fundo e pela cor. */}
                                        {isActive && (
                                            <div className="hidden md:block absolute left-[-12px] top-1/2 -translate-y-1/2 w-1 h-5 bg-brand-600 rounded-r-full" />
                                        )}
                                    </Link>
                                );
            ...
            <div className="flex-1 flex flex-col min-w-0 md:overflow-hidden">
                ...
                <main className="flex-1 md:overflow-auto p-4 md:p-6 bg-gray-100">
